...
namespace GoGreenV3.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ...
            var agencyAddress = from a in agencyDb.Agencies select a;
            //ViewBag.AgencyAddress = agencyAddress.ToString();

            foreach (var item in agencyAddress.ToList())
            {
                if (item.Name == user.Agency)
                {
                    ViewBag.AgencyAddress = agencyDb.Agencies.Find(item.Id).Address;
                    break;
                }
            }
